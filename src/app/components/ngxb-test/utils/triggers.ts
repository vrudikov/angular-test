...
export function listenToTriggersV2(renderer: Renderer2, options: ListenOptions): Function {
  ...
  parsedTriggers.forEach((trigger: Trigger) => {
    ...
    if (!useToggle) {
      _registerHide.push(() => renderer.listen(target, trigger.close, <(event: any) => boolean>options.hide));
    }
...
export function registerOutsideClick(renderer: Renderer2, options: ListenOptions) {
