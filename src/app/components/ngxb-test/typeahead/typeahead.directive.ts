...
@Directive({selector: '[appTypeahead2]', exportAs: 'bs-typeahead'})
export class TypeaheadDirective implements OnInit, OnDestroy {
   ...
   * When set to 0, typeahead shows on focus with full list of options (limited as normal by typeaheadOptionsLimit) */
  ...
  /** when options source is an array of objects, the name of field that contains the options value,
   * we use array item as option in case of this field is missing. Supports nested properties and methods. */
  @Input() public typeaheadOptionField: string;
  /** when options source is an array of objects, the name of field that contains the group value,
   * matches are grouped by this field when set. */
  @Input() public typeaheadGroupField: string;
  /** should be used only in case of typeahead attribute is array. If true - loading of options will be async,
   * otherwise - sync. true make sense if options array is large. */
  ...
  /** break words with spaces. If true the text "exact phrase" here match would match with match exact phrase here
  ...
  /** should be used only in case typeaheadSingleWords attribute is true. Sets the word delimiter to match exact phrase.
  ...
  public constructor(control: NgControl, viewContainerRef: ViewContainerRef, element: ElementRef,
                     renderer: Renderer2, cis: ComponentLoaderFactory) {
